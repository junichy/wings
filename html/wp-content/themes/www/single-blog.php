<?php get_header(); ?>

<main class="article">

  <!-- headline -->
  <?php get_template_part('parts/headline') ?>

  <section id="s-blog" class="s-blog single">

    <div class="single__inner section__inner page__inner">
      <div class="single__main_column">
        <!-- info-top -->
        <div class="single__info-top">
          <?php 
            $taxonomy = 'cat_blog'; //カスタムタクソノミースラッグ
            $taxonomy_terms_cat = get_the_terms($post->ID, $taxonomy);
            $taxonomy_terms_cat_name = $taxonomy_terms_cat[0]->name; //ターム名
            $taxonomy_terms_cat_slug = $taxonomy_terms_cat[0]->slug; //タームスラッグ
          ?>
          <p class="single__info--cat"><a href="<?php echo esc_url( home_url() ); ?>/cat_blog/<?php echo $taxonomy_terms_cat_slug; ?>"><?php echo $taxonomy_terms_cat_name; ?></a></p>
          <p class="single__info--date">
          <?php echo get_the_date("Y.m.d"); ?>
          </p>
        </div>

        <!-- title -->
        <h1 class="single__title"><?php the_title(); ?></h1>

        <!-- thumbnail -->
        <?php if (has_post_thumbnail()) : ?>
          <div class="single__thumbnail">
              <?php the_post_thumbnail('full'); ?>
          </div>
        <?php endif; ?>

        <!-- content -->
        <div class="single__content">
          <div class="single__content-inner">
            <?php the_content(); ?>
          </div>
        </div>
        <p class="single__content-btn btn btn__white"><a href="<?php echo esc_url( home_url() ); ?>/blog/"><span>ブログ一覧を見る</span></a></p>
      </div>
      <div class="single__side_column">
        <?php echo get_template_part('parts/sidebar') ?>
      </div>
  </section>

</main>

<?php get_footer(); ?>